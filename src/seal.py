...
from src.seal_utils import train_test_split_edges
...
data = Data(x = features, y = labels, edge_index = edge_list, edge_type = edge_type)
...
class SEALDataset(InMemoryDataset):
    def __init__(self, data, num_hops, split='train'):
        ...
        # self.data.x = torch.zeros(self.data.x.size())
    ...
    def processed_dir(self):
        return osp.join(self.root, 'dataset', 'processed')
    ...
    def process(self):
        ...
        print("Generating testing subgraphs...")
...
# test_dataset = SEALDataset(data, num_hops=1, split='test')
...
def test(data):
    data = data.to(device)
    outputs = model(data.x, data.edge_index).view(-1)
    
    return F.sigmoid(outputs).cpu().numpy()
...
# TODO: Generate test outputs
test_data = None
test_outputs = test(test_data)

now_time = time.strftime("%m%d%H%M%S", time.localtime(int(time.time())))
pd.DataFrame(
    np.hstack([np.arange(test_outputs.shape[0]), test_outputs])
).to_csv("submission/submission_{}.csv".format(now_time), index=False)