...
adj, features, labels, idx_train, idx_val, idx_test, loss_coef = load_author_data()
...
test_pos_edge_index = torch.hstack([data.train_pos_edge_index, data.val_pos_edge_index]).to(device)
output_pos_edge_index = torch.hstack([data.train_pos_edge_index,
...
def get_test_output():
    ...
    edge_index = (torch.LongTensor(pairs_to_pred)).to(device)
    with torch.no_grad():
        z = model.encode(x, output_pos_edge_index)
        pred = model.decode(z, edge_index, sigmoid=True)
        
    return pred.view(-1).cpu().numpy(), z.cpu().numpy()
...
for epoch in range(1, args.epochs + 1):
    ...
    if auc > best_auc:
        best_auc = auc
        test_output, embedding = get_test_output()
    print('Epoch: {:03d}, AUC: {:.4f}, AP: {:.4f}'.format(epoch, auc, ap))

now_time = time.strftime("%m%d%H%M%S", time.localtime(int(time.time())))
model_type = "VGAE" if args.variational else "GAE"
create_submission(test_output, model_type, now_time)

# np.save("dataset/vgae_embedding.npy", embedding)