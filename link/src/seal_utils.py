...
def train_val_split_edges_n2v(data, val_ratio=0.05):
    r"""Splits the edges of a :obj:`torch_geometric.data.Data` object
    into positive and negative train/val/test edges, and adds attributes of
    `train_pos_edge_index`, `train_neg_adj_mask`, `val_pos_edge_index`,
    and `val_neg_edge_index` to :attr:`data`.
    ...
    data.val_pos_edge_label = data.edge_label[perm[:n_v]]

    r, c = row[n_v:], col[n_v:]
    data.train_pos_edge_index = torch.stack([r, c], dim=0)
    data.train_pos_edge_index = to_undirected(data.train_pos_edge_index)
    data.train_pos_edge_label = data.edge_label[perm[n_v:]]
    
    # data.graph_edge_index = to_undirected(data.graph_edge_index)
    data.train_edge_index = to_undirected(data.train_edge_index)

    return data


def train_val_split_edges(data, val_ratio=0.05):
    r"""Splits the edges of a :obj:`torch_geometric.data.Data` object
    into positive and negative train/val/test edges, and adds attributes of
    `train_pos_edge_index`, `train_neg_adj_mask`, `val_pos_edge_index`,
    and `val_neg_edge_index` to :attr:`data`.
